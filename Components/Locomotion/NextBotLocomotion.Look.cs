...
partial class NextBotLocomotion
{
	...
	public virtual void UpkeepAim()
	{
		...
		var origin = Bot.EyePosition;
		var angles = Bot.EyeRotation;
		var forward = angles.Forward;

		var deltaAngle = angles.Distance( PriorAim );
		AimRate = MathF.Abs( deltaAngle / deltaT );
		PriorAim = angles;
		...
		var newAngles = Rotation.Lerp( angles, desiredAngles, approachRate * deltaT );
		Bot.NextBot.FaceTowards( newAngles );
	}
	...
	public virtual void FaceTowards( Vector3 target ) { }
